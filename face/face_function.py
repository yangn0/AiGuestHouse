from face import face_dll,face_class
from ctypes import *
import cv2
...
def CSH():# 1：视频或图片模式,2角度,3最小人脸尺寸推荐16,4最多人脸数最大50,5功能,6返回激活句柄
    ret=face_dll.chushihua(0xFFFFFFFF,0x5,16,50,5,byref(Handle))
    # Main.Handle=Handle
    return ret,Handle
# cv2记载图片并处理
def LoadImg(im):
...
def VF(frame):
    ...
    imgby=bytes(img)
    imgcuby=cast(imgby,c_ubyte_p)
    ret=face_dll.shibie(Handle,img.shape[1],img.shape[0],0x201,imgcuby,byref(faces))
    return ret,faces

def draw_div(frame,faces):
    for i in range(0,faces.faceNum):    
        ra=faces.faceRect[i]
        cv2.rectangle(frame,(ra.left1,ra.top1),(ra.right1,ra.bottom1),(255,0,0,),2)
    return frame
...
def RLTZ(frame,ft):
    ...
    imgby=bytes(frame)
    imgcuby=cast(imgby,c_ubyte_p)
    ret=face_dll.tezheng(Handle,img.shape[1],img.shape[0],0x201,imgcuby,ft,byref(detectedFaces))
    if ret==0:
